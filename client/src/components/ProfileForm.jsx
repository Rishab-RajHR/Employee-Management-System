...
const ProfileForm = ({initialData, onSuccess}) => {
  ...
  return (
    <form onSubmit={handleSubmit} className='card p-5 sm:p-6 mb-6'>
        ...
        {message && (
            <div className='bg-emerald-50 text-emerald-700 p-4 rounded-xl text-sm border border-emerald-200 mb-2 flex items-start gap-3'>
                <div className='w-1.5 h-1.5 rounded-full bg-rose-500 mt-1.5 shrink-0'/>
                {message}
            </div>
        )}
        ...
        <div className='space-y-5'>
            <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
                 <div>
                      ...
                      <input disabled value={`${initialData.first}`}className='bg-slate-50 text-slate-400 cursor-not-allowed'/>
                 </div>
                 <div>
                      <label className='block text-sm font-medium text-slate-700 mb-2'>
                         Email
